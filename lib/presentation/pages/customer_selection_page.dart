...
class _CustomerSelectionPageState extends State<CustomerSelectionPage> {
  ...
  Future<void> _search([String? raw]) async {
    ...
    final query = ThirdPartyQuery(
      page: '1',
      amount: '15',
      // Si parece documento → identificationNumber; si no → companyName
      identificationNumber: _looksLikeDocument(text) ? text : null,
      companyName: !_looksLikeDocument(text) && text.isNotEmpty ? text : null,
    );

    await context.read<ThirdPartyStore>().loadThirdParties(query: query);
  }

  bool _looksLikeDocument(String text) {
    if (text.isEmpty) return false;
    final digits = text.replaceAll(RegExp(r'[\s.-]'), '');
    return RegExp(r'^\d+$').hasMatch(digits) && digits.length >= 5;
  }
